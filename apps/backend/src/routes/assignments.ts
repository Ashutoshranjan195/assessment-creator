...
assignmentsRouter.post('/', async (req, res, next) => {
  try {
    ...
    const questionTypes = parsed.questionTypes.map((q) => q.type);
    const numberOfQuestions = parsed.questionTypes.reduce((s, q) => s + q.count, 0);
    const totalMarks = parsed.questionTypes.reduce((s, q) => s + q.count * q.marksEach, 0);
